...
def _attach_request_scoped_context(
    ...
    *,
    ...
    context = DiffusionKVContext(
        context_id=context_id or f"{request.request_id}-ar",
...
class TestRequestScheduler:
    ...
    def test_request_scoped_context_request_id_reuse_gets_new_generation(self) -> None:
        ...
        _attach_request_scoped_context(first, context_id="old-ar")
        manager = self.scheduler._diffusion_kv_manager
        assert manager is not None
        free_before = manager.native_manager.block_pool.get_num_free_blocks()
        ...
        assert "reused/diffusion-kv/context/old-ar" not in manager._internal_request_ids

        second = _make_request("reused")
        _attach_request_scoped_context(second, context_id="new-ar")
        self.scheduler.add_request(second)
        second_output = self.scheduler.schedule()
        ...
        assert [context.context_id for context in second_metadata.contexts] == ["new-ar"]
        assert "reused/diffusion-kv/context/old-ar" not in manager._internal_request_ids
        assert "reused/diffusion-kv/context/new-ar" in manager._internal_request_ids
