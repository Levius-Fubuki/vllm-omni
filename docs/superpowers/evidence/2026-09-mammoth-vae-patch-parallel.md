...
| --- | ---: | ---: | --- |
| 1280×1024, 2 denoise steps | 139.708 / 136.785 / 2.790 s | 141.090 / 138.028 / 2.941 s | AR token IDs identical; RGB mean absolute pixel difference 0.361/255, maximum 8/255 |
| 2048×2048, 20 denoise steps | 560.324 / 453.037 / 107.054 s | 543.023 / 435.624 / 107.123 s | AR token IDs identical; RGB mean absolute pixel difference 0.722/255, maximum 50/255 |

Both 2048² requests generated all 16,513 expected AR image tokens with `max_model_len=18432`; their identical AR token-ID sequence has SHA-256 `c7842b0278be06211ca6dc512d288f14cc1c85a134ee61bdeafd5999fa2ef4c1`. The `finish_reason='length'` here reflects the explicitly requested 16,513 tokens, not context truncation. The output PNG SHA-256 values are `176b1002d321c785773bcf5bdd0705afb388a53a5fc030747cebd364f8fdd958` (PP=1) and `5cdde82c5c04e95de431ffa8c1b9dc772a3be5b2b6610dad4931aaf32f67e9d1` (PP=2). Both are valid 2048×2048 RGB images. However, AR hidden states were not persisted; equal token IDs do not prove equal DiT inputs, and the image difference is not exclusively attributable to VAE decode. Visual inspection found no obvious PP=2 seam, but a single image is not a quality study.

The matched 2048² configurations are [PP=1](mammoth-4gpu-2048-pp1.yaml) and [PP=2](mammoth-4gpu-2048-pp2.yaml); the 1280×1024 PP=2 configuration is [here](mammoth-4gpu-pp2.yaml), with [the existing PP=1 configuration](mammoth-3gpu-baseline.yaml). The 2048² request initially used `max_model_len=6144` and stopped AR after 6,110 output tokens. Those earlier images are **invalid as full-grid comparisons** and excluded from this table. The complete 2048² stage-1 times differ by only 0.069 s, while AR differed by 17.413 s. Thus this paired run does **not** establish request-level latency improvement. Omni reported primary diffusion-worker peak **reserved** memory of 22,374 MiB (PP=1) and 11,674 MiB (PP=2); this is a stage-configuration observation, not a VAE-only saving and not necessarily the maximum across all workers.

On the same four-GPU host's 2↔3 PXB pair, a separate real-weight FP16 VAE-only 2048² run (one warmup, three timed iterations) measured untiled PP=1 median 1222.1 ms, tiled PP=1 1693.1 ms and tiled PP=2 950.5 ms: **1.78× versus like-for-like tiled decode**. The two tiled outputs were bit-identical in this run, including actual cross-rank tile boundaries; both modes repeated exactly. PP=1 tiled versus PP=2 peak *allocated* memory was approximately 2.78 GB on rank 0 versus 2.77/2.74 GB on ranks 0/1. The [raw JSON](mammoth-vae-2048-4gpu-pxb-cu129.json) includes source hashes, exact timings and boundary mapping. Allocator-cache-retained peak *reserved* memory is not comparable across modes. This isolated decode benefit is compatible with the flat complete-request stage-1 result: the latter is dominated by 20 DiT steps and includes SP=2 overhead.
