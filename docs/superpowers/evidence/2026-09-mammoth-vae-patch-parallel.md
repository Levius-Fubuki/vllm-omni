...
Date: 2026-09-19. This includes VAE-only performance measurements, a two-rank stage-construction/DiT-forward/VAE-decode smoke test, and a single-rank full AR→DiT→VAE inference result. It does not include a full AR→DiT request with two-rank VAE patch parallelism.
...
2. Validate one complete Preview AR→DiT generation with **PP=2** on hardware able to concurrently host AR TP=2 and two DiT/VAE ranks; three 24 GiB 3090s could run the full PP=1 request but not the shared-GPU PP=2 placement. Until then the ~1.7–1.8× result is VAE-only, not a proven request-level speedup.
